locals {
  k3s-install-command = "curl -sfL https://get.k3s.io | K3S_TOKEN=${nonsensitive(random_password.k3s_secret.result)} K3S_KUBECONFIG_MODE=${var.k3s-config-mode} sh -s -"
...
resource "null_resource" "install_kubevip" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo ctr image pull ghcr.io/kube-vip/kube-vip:latest; sudo ctr run --rm --net-host ghcr.io/kube-vip/kube-vip:latest vip /kube-vip manifest daemonset --arp --interface eth0 --address ${var.k3s-controlplane-ip} --controlplane --inCluster --taint --leaderElection --services  | kubectl apply -f -",
